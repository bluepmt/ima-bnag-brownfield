...
docs/                                # Tài liệu vận hành
  ...
  └─ IMA_Progress.xlsx               # Workbook mẫu Progress (9 named tables) → upload OneDrive cho Graph
...
- Progress (trang 08): kéo từ Excel 365 trên OneDrive qua Microsoft Graph — `functions/_lib/graph.js` + `GET /api/progress`. Workbook `docs/IMA_Progress.xlsx` gồm **9 named tables** (`KPI, Phase, Sections, Variance, Milestones, Engineering, Procurement, SCurve, WPInfo`); `graph.js` đọc từng bảng theo tên (`/tables('NAME')/range`) → trả `{ tables, errors, fetchedAt }`. Trang 08 (`applyLiveProgress()`) ghi đè data mô phỏng `EPCI_DATA` khi có data thật, fallback mô phỏng khi 503/lỗi. Layout cột: xem `docs/BACKEND_SETUP.md` §G. (Env cũ `GRAPH_TABLE` bỏ → optional `GRAPH_TABLES`.)
...
- Secrets (`GRAPH_*`, API key) chỉ ở Cloudflare env/Secrets — KHÔNG commit. `.dev.vars`, `seed.sql` đã nằm trong `.gitignore`.
...
### Data model + live overlay (Excel 365)
...
- **KHÔNG** chặn render bằng fetch (phải render mô phỏng trước, overlay sau). **KHÔNG** reassign các const (mutate tại chỗ). Thêm equipment/field mới phải đồng bộ cả Excel template (`docs/IMA_Progress.xlsx` + `db/build_progress_xlsx.cjs`) lẫn mapping `applyLiveProgress()`.
...
- **Không để `loadLiveProgress()` chặn render trang 08, và không reassign các const data** (`EPCI_DATA`, `PHYS_SCURVE`…) — phải render mô phỏng trước rồi overlay, và mutate tại chỗ. Thêm field/tuần mới phải đồng bộ cả `docs/IMA_Progress.xlsx` (+`db/build_progress_xlsx.cjs`) lẫn `applyLiveProgress()`. Đổi shape table phải sửa cả `functions/_lib/graph.js`.
- **Không đổi `functions/_lib/graph.js` về đọc 1 bảng `Key|Value`** — đã chuyển sang đọc 9 named tables (`/tables('NAME')/range`) trả `{ tables, errors, fetchedAt }`. Env `GRAPH_TABLE` cũ bỏ (dùng `GRAPH_TABLES` optional).
